...
class Total extends \Magetrend\PdfTemplates\Model\Pdf\Element
{
    ...
    public function draw($pdfPage, $elemetData, $source, $template, $elements, $currentPage)
    {
        ...
        if ($this->getSource() instanceof \Magento\Quote\Model\Quote) {
            $totalData = $this->totalHelper->getQuoteTotalData($attributes, $source, $template);
        } else {
            $totalData = $this->totalHelper->getOrderTotalData($attributes, $order, $source, $template);
        }
    ...
    public function getLabelAligment()
    {
        return 'left';
    }
