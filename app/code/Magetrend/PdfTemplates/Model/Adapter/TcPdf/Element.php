...
class Element
{
        ...
        foreach ($lines as $line) {
            ...
            try {
                ...
                if ($this->moduleHelper->isPriceFixEnabled($this->getSource()->getStoreId())) {
                    ...
                    if (!empty($separators)) {
                       $this->writeWithSeparators($lineHeight, $line, $separators);
                    } else {
                        $this->pdf->Write($lineHeight, $line, '', false, 'L', false);
                    }
                }
    ...
    public function writeWithSeparators($lineHeight, $line, $separators)
    {
        foreach ($separators as  $key => $sep) {
            $line = str_replace($sep, '{sep}{index'.$key.'}{/sep}', $line);
        }

        $line = explode('{/sep}', $line);
        $lineParts = [];
        foreach ($line as $linePart) {
            $linePart = explode('{sep}', $linePart);
            $lineParts = array_merge($lineParts, $linePart);
        }

        foreach ($lineParts as $key =>  $part) {
            if (strpos($part, '{index') === false) {
                continue;
            }

            $separatorIndex = str_replace(['{index', '}'], '', $part);
            $lineParts[$key] = $separators[$separatorIndex];
        }
        ...
        foreach ($lineParts as $part) {
            ...
            if ($this->isRTL($part)) {
                $lineParts = array_reverse($lineParts);
            }
        ...
        foreach ($lineParts as $part) {
            ...
            $this->pdf->Write($lineHeight, $part, '', false, 'L', false);
        }
    }
