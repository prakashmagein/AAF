...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    const XML_PATH_SPLIT_PRODUCT_OPTION = 'pdftemplates/general/split_product_option';
    ...
    const MOVE_LAST_ITEM = 'pdftemplates/additional/move_last_item';
    ...
    const XML_PATH_ENCODE_QR = 'pdftemplates/additional/qr_base64';
    ...
    const PDF_TEMPLATE_TMP_DIR = 'pdftemplates';
    ...
    /**
     * @var \Magetrend\PdfTemplates\Model\Config\Media\Config
     */
    public $mediaConfig;
     ...
     * @param \Magetrend\PdfTemplates\Model\Config\Media\Config $mediaConfig
     */
    public function __construct(
        ...
        \Magetrend\PdfTemplates\Model\Config\Media\Config $mediaConfig
    ) {
    ...
    public function encodeQRBase64($storeId = null)
    {
        return $this->scopeConfig->getValue(
            self::XML_PATH_ENCODE_QR,
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
            $storeId
        );
    }
    ...
    public function moveLastItem($storeId = 0)
    {
        return $this->scopeConfig->getValue(
            self::MOVE_LAST_ITEM,
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
    ...
    public function getTotalsHideList($store = null)
    {
        $totalsConfig = $this->scopeConfig->getValue(
            'pdftemplates/additional/total_sort_order',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
            $store
        );
        ...
        $hideList = [];
        if (!empty($totalsConfig)) {
            foreach ($totalsConfig as $totalLine) {
                if (isset($totalLine['hide_field']) && $totalLine['hide_field'] == 1) {
                    $hideList[] = $totalLine['source_field'];
                }
            }
        }
    ...
    public function removeInterlaceFromPng($path, $saveToTmp = false)
    {
        ...
        if ($saveToTmp) {
            ...
            $destination->create(self::PDF_TEMPLATE_TMP_DIR);
            $ext = explode('.', $path);
            $ext = end($ext);
            $uniqueId = uniqid(\Magento\Framework\Math\Random::getRandomNumber()) . time() . '.'.$ext;
            $path = $destination->getAbsolutePath(self::PDF_TEMPLATE_TMP_DIR.'/'.'nonInterlacepng_'.$uniqueId);
        }
    ...
    public function splitProductOption($store = null)
    {
        return $this->scopeConfig->getValue(
            self::XML_PATH_SPLIT_PRODUCT_OPTION,
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
            $store
        );
    }
    ...
    /**
     * Copy file to tmp from storage
     * @param $path
     * @param $file
     * @return \Magento\Framework\Filesystem\Directory\WriteInterface
     * @throws \Magento\Framework\Exception\FileSystemException
     */
    public function makeTmpCopy($path, $file)
    {
        try {
            $readDir = $this->filesystem->getDirectoryReadByPath($path);
            $tmpDir = $this->filesystem->getDirectoryWrite(DirectoryList::TMP);
            $tmpDir->create(self::PDF_TEMPLATE_TMP_DIR);
            $tmpFile = $tmpDir->getAbsolutePath(self::PDF_TEMPLATE_TMP_DIR.'/'.$file);
            $tmpDir->writeFile($tmpFile, $readDir->readFile($file));
        } catch (\Magento\Framework\Exception\FileSystemException $e) {
            return false;
        }

        return [
            'path' => $tmpDir->getAbsolutePath(self::PDF_TEMPLATE_TMP_DIR),
            'dir' => $tmpDir,
            'file' => $file,
            'file_path' => $tmpFile
        ];
    ...
    /**
     * Copy file back to storage after GD operations
     * @param $file
     * @param $path
     * @throws \Magento\Framework\Exception\FileSystemException
     */
    public function moveTmpCopyToMedia($file, $path)
    {
        $filePath = $this->filesystem->getDirectoryReadByPath($path)->getAbsolutePath($file);
        $mediaWrite = $this->filesystem->getDirectoryWrite(DirectoryList::MEDIA);
        $tmpDir = $this->filesystem->getDirectoryRead(DirectoryList::TMP);
        $tmpFile = $tmpDir->getAbsolutePath(self::PDF_TEMPLATE_TMP_DIR.'/'.$file);
        $mediaWrite->writeFile($filePath, $tmpDir->readFile($tmpFile));
    }
     ...
     * @throws \Magento\Framework\Exception\FileSystemException
     */
    public function deleteTmpCopy($file)
    {
        $this->filesystem->getDirectoryWrite(DirectoryList::TMP)
            ->delete(self::PDF_TEMPLATE_TMP_DIR.'/'.$file);
    }

    /**
     * Make a local copy of image if an image is in remote storage
     * @param $imagePath
     * @return string
     */
    public function resolveImagePath($imagePath)
    {
        if (strpos($imagePath, 'https://') === null && strpos($imagePath, 'http://') === null) {
            return $imagePath;
        }

        $imgSplit = explode('/', $imagePath);
        $fileName = end($imgSplit);
        $path = str_replace($fileName, '', $imagePath);
        $tmpCopy = $this->makeTmpCopy($path, $fileName);
        if (!$tmpCopy) {
            return false;
        }
        return rtrim($tmpCopy['path'], '/').'/'.$fileName;
    }

    public function cleanTmpDir()
    {
        $this->filesystem->getDirectoryWrite(DirectoryList::TMP)
            ->delete(self::PDF_TEMPLATE_TMP_DIR);
    }
