...
class Verify extends Action
{
    ...
    public function execute()
    {
        ...
        $toNumber = isset($_data['mobile']) ? $_data['mobile'] : "";
        if ($this->datahelper->isCustomerCountryEnabled() && isset($_data['countrycode'])) {
            $toNumber = $_data['countrycode'].$_data['mobile'];
        }

        if (isset($_data['email'])) {
            $toNumber = $_data['email'];
        }
        ...
        if ($smsProfileOtp->getSize()) {
            ...
            if ($data->getOtpCode() == $otp) {
                $result->setData(['success' =>'Verified', 'message' => __('Verified')]);
            } else {
