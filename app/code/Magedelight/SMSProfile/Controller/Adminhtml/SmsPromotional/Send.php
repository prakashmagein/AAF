    ...
    public function getToNumers($data)
    {
        ...
        switch ($sendsms) {
            ...
            case 'customer_group':
                ...
                break;
            case 'custom':
                ($data['tonumbers'] != null) ? $toNumber = (explode(",", $data['tonumbers'])) : $toNumber =[] ;
                break;
            case 'abandoned_cart':
            ...
            foreach ($importRawData as $import) {
                if (is_numeric($import[0])) {
                    if (strpos($import[0], '+') === false && $this->getApiVersion() == 'Twilio Api Service') {
                        $no[] =  '+'.$import[0];
                    } elseif (strpos($import[0], '+') === false && $this->datahelper->getSmsProfileApiCountryRequired() && $this->getApiVersion() != 'Twilio Api Service') {
                         $no[] =  '+'.$import[0];
                    } else {
                        $no[] = $import[0];
                    }
