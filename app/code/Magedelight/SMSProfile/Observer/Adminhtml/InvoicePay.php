...
class InvoicePay implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        ...
        $customerEvent = 'customer_invoice';
        $customerEventList  = $this->datahelper->getCustomerEvents();
        if ($this->datahelper->getModuleStatus($order->getStoreId()) && (in_array($customerEvent, $customerEventList)) && $order->getShippingAddress()) {
            if (is_array($this->getToNumber($order))) {
                foreach ($this->getToNumber($order) as $toNumber) {
                    if ($this->datahelper->getSendOtpVia() !='1') {
                        ...
                        $this->smsNotificationService->setMessageContent($this->getMessageText($invoice, $customerEvent));
