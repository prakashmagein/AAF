...
class Price extends AbstractFilter
{
    ...
    public function apply(\Magento\Framework\App\RequestInterface $request)
    {
        ...
        list($from, $to) = $filter;
        $taxratevalues = $this->getTaxRates();
        if(isset($taxratevalues) && !empty($taxratevalues)){
            $fromnew = $from/(1+($taxratevalues/100));
            $tonew = $to/(1+($taxratevalues/100));
        }

        $fromBase = (float) $fromnew/$this->getCurrencyRate();
        $toBase = (float) $tonew/$this->getCurrencyRate();
