<?php
namespace Magepow\ProductTags\Model\ResourceModel;
...
class Tag extends AbstractDb
{
    ...
    {
        $this->_init('magepow_producttags_tag', 'tag_id');
    }
     ...
     * @throws LocalizedException
     */
    protected function _afterSave(AbstractModel $tag)
    {
     ...
     public function getTagProductTable()
    {
        if (!$this->_tagProductTable) {
            $this->_tagProductTable = $this->getTable('magepow_producttags_product');
        }
    ...
    protected function _saveTagProducts($tag)
    {
        ...
        if (!empty($update)) {
            ...
            foreach ($update as $productId => $position) {
                $delta = (int) $position - $oldProducts[$productId];
        ...
        $tableName = $this->getTable('magepow_producttags_tag'); //gives table name with prefix
        $connection->update(
    ...
    {
        $productTable = $this->getTable('magepow_producttags_product');
    ...
    protected function _saveTagStores($tag)
    {
        ...
        $table = $this->getTable('magepow_producttags_store');
    ...
    public function lookupStoreIds($tagId)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('magepow_producttags_store'),
            'store_id'
            )
        ->where(
            'tag_id = ?',
    ...
    protected function _beforeDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('magepow_producttags_store'), $condition);
        $condition = ['tag_id = ?' => (int)$object->getId()];
        $this->getConnection()->delete($this->getTable('magepow_producttags_product'), $condition);
        return parent::_beforeDelete($object);
    }
    /**
     * Get positions of associated to tag products
     *
     * @param \Magepow\ProductTags\Model\Tag $tag
     * @return array
     */
    public function getProductsPosition($tag)
    {
        $select = $this->getConnection()->select()->from(
            ...
            "{$this->getTable('magepow_producttags_product')}.tag_id = ?",
            (int)$tag->getId()
        );
    ...
    public function getTagIdByIdentifier($tag_code = ""){
        if($tag_code) {
            ...
            $select = $this->getConnection()->select()->from($this->getMainTable(), 'tag_id')->where(
                "{$this->getTable('magepow_producttags_tag')}.identifier = ?",
                $tag_code
            );
    ...
    protected function _getLoadByIdentifierSelect($identifier, $store, $isActive = null)
    {
        ...
        $select = $this->getConnection()->select()
            ...
            ->join(
                ['cps' => $this->getTable('magepow_producttags_store')],
