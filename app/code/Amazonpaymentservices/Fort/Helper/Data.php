...
class Data extends \Magento\Payment\Helper\Data
{
    ...
    public function capturePayment($responseParams)
    {
        ...
        if ($paymentMethod == \Amazonpaymentservices\Fort\Model\Method\Apple::CODE) {
            ...
            if (strpos($baseUrl, 'miro-sa.com') !== false) {
                $postData = [
                    'merchant_reference'    => $orderId,
                    'access_code'           => $this->getMainConfigData('access_code'),
                    'command'               => 'CAPTURE',
                    'merchant_identifier'   => 'LWADMiNB',
                    'amount'                => $amount,
                    'currency'              => strtoupper($orderCurrency),
                    'language'              => $language
                ]; 
...
//            $order->registerCancellation($comment)->save();
    ...
    public function getTabbyRequestParams($order, $integrationType)
    {
        ...
        $gatewayParams['command']        = \Amazonpaymentservices\Fort\Model\Config\Source\Commandoptions::PURCHASE;
        $gatewayParams['payment_option'] = 'TABBY';
        $gatewayParams['customer_ip']    = $this->getVisitorIp();
        ...
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/tabbyrequest.log');
        $logger = new \Zend_Log();
        $logger->addWriter($writer);
        $logger->info('Request Param: ' . json_encode($gatewayParams));

        $gatewayUrl = $this->getGatewayUrl();

        // Log Gateway URL separately
        $urlWriter = new \Zend_Log_Writer_Stream(BP . '/var/log/tabbygatewayurl.log');
        $urlLogger = new \Zend_Log();
        $urlLogger->addWriter($urlWriter);
        $urlLogger->info('Gateway URL: ' . $gatewayUrl);

        // Also log both together for internal module logs
        $logMsg = "Request Params for payment method (TABBY)\n\n" . json_encode($gatewayParams, JSON_PRETTY_PRINT);
        $this->log($logMsg);
