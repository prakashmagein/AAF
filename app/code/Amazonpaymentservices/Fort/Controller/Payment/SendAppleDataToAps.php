...
class SendAppleDataToAps extends \Amazonpaymentservices\Fort\Controller\Checkout implements CsrfAwareActionInterface, HttpGetActionInterface, HttpPostActionInterface
{
	...
	$writer = new \Zend_Log_Writer_Stream(BP . '/var/log/paymentnew.log');
                    $logger = new \Zend_Log();
                    $logger->addWriter($writer);
                    $logger->info(json_encode($jsonData));
        $helper = $this->getHelper();
        $response = $helper->applePayResponse($jsonData);
