...
use Amasty\Feed\Controller\Adminhtml\Feed\Connection;
...
class Ftp
{
    ...
    public function ftpUpload(FeedInterface $feed)
    {
        ...
        if (strpos($feed->getDeliveryHost(), ':') !== false) {
            [$host, $port] = explode(':', $feed->getDeliveryHost(), 2);
        } else {
    ...
    public function sftpUpload(FeedInterface $feed)
    {
        ...
        if (version_compare($this->magentoVersion->get(), '2.2.0', '<')) {
            ...
            define('NET_SFTP_LOCAL_FILE', Connection::SOURCE_LOCAL_FILE);
            define('NET_SFTP_STRING', Connection::SOURCE_STRING);
        }
