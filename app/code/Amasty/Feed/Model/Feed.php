...
use Amasty\Feed\Api\Data\FeedExtensionInterface;
...
class Feed extends AbstractExtensibleModel implements FeedInterface
{
     ...
     * @var string
     */
    protected $_eventPrefix = 'amasty_feed';
            ...
            if (is_array($config)) {
                foreach ($config as $item) {
                    $ret[] = [
                        ...
                        'static_text' => $item['static_text'] ?? null,
                        'format' => $item['format'] ?? '',
                        'parent' => $item['parent'] ?? '',
                        'modify' => $item['modify'] ?? [],
                    ];
    ...
    public function getConditionsSerialized()
    {
        ...
        if ($conditionsSerialized && $conditionsSerialized[0] === 'a' // Old serialization format used
            && interface_exists(SerializerInterface::class) // New version of Magento
        ) {
    ...
    {
        return $this->_getData(FeedInterface::TEMPLATE_CODE);
    }

    public function setTemplateCode(string $templateCode): void
    {
        $this->setData(FeedInterface::TEMPLATE_CODE, $templateCode);
    }
