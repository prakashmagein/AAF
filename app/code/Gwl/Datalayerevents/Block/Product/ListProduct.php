    ...
    public function getViewItemList($productCollection)
    {
        if ($productCollection->count()) {
            ...
            foreach ($productCollection as $product) {
                ...
                $item["price"] = round($product->getPriceInfo()->getPrice('final_price')->getMinimalPrice()->getValue(),2);
                }
                
                $item['original_price'] = (float)($product->getPrice());

                if ($product->getTypeId() == "bundle") {
                $item['original_price'] = round($product->getPriceInfo()->getPrice('final_price')->getMinimalPrice()->getValue(),2);
                }
